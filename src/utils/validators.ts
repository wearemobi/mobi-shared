...
export const mobiValidators = {
  ...
  minLength: (minLen: number, msg?: string): ValidatorFn => 
    (value: any) => {
      if (!value) return null;
      return value.length < minLen ? (msg || `Must be at least ${minLen} characters`) : null;
    },
  ...
  maxLength: (maxLen: number, msg?: string): ValidatorFn => 
    (value: any) => {
      if (!value) return null;
      return value.length > maxLen ? (msg || `Must be at most ${maxLen} characters`) : null;
    },
  ...
  money: (msg = 'Value must be a valid money format'): ValidatorFn => 
    (value: any) => {
      ...
      if (decimalIndex !== -1 && cleaned.split('.')[1].length > 2) {
        return msg;
      }
      return null;
    }
